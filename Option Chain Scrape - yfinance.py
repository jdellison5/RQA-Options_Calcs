...
if "jdell" in cdir:
    ...
    base_path = "C:\\Users\\AHolp\\OneDrive - Richmond Quantitative Advisors\\The Answer 3.10\\" 
...
opt_v_share_vol = (df['volume'].sum() * 100) / price_data['Volume'][-1] #When over 0.4, NOPE_MAD provides a fairly good windo into predicting earnings behavior.
...
option_effects_list = [time_stamp, delta_exposure, gamma_exposure, opt_v_share_vol, nope]
option_effects_df = pd.DataFrame(option_effects_list).transpose()
option_effects_df.columns = ['Date/Time','Delta_Exposure','Gamma_Exposure','Opt_v_Share_Volume', 'NOPE']
